...
public class SeckillGoodsController {
    ...
    public Result<List<Date>> dateMenus() {
        ...
        return new Result<List<Date>>(true, StatusCode.OK, "秒杀时间菜单查询成功", dateMenus);
    }

    /****
     * 对应时间段秒杀商品集合查询
     * @param time:2021050716
     */
    @RequestMapping(value = "/list")
    public Result<List<SeckillGoods>> list(String time) {
        //调用Service查询数据
        List<SeckillGoods> SeckillGoodsList = seckillGoodsService.list(time);
        return new Result<List<SeckillGoods>>(true, StatusCode.OK, "秒杀商品集合查询成功", SeckillGoodsList);
    }
