...
public class OrderServiceImpl implements OrderService {
    ...
    public void updateStatus(String outTradeNo, String payTime, String transactionId) throws Exception {
        ...
        Date payTimeDate = simpleDateFormat.parse(payTime);

        // 1. 查询订单
        Order order = orderMapper.selectByPrimaryKey(outTradeNo);

        // 2. 修改数据
        order.setPayTime(payTimeDate); // 支付时间
    ...
    public void add(Order order) {
        ...
        Map<String, Integer> decrMap = new HashMap<>(); // 封装库存递减数据
        ...
        order.setPayMoney(totalMoney); // 实付金额 实付金额-优惠价格的时候 就不一样
        ...
        String sendStartTime = simpleDateFormat.format(new Date());
        System.out.println("创建订单时间 : " + sendStartTime);
