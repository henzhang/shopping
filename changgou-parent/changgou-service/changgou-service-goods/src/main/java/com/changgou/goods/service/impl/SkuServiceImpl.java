...
public class SkuServiceImpl implements SkuService {
    ...
    public void decrCount(Map<String, Integer> decrMap) {
        for (Map.Entry<String, Integer> entry : decrMap.entrySet()) {
            Long id = Long.valueOf(entry.getKey()); // 商品id
            Object obj = entry.getValue();
            Integer num = Integer.valueOf(obj.toString()); // 商品递减数量

            /**
             * 库存数量 >= 递减数量 张三  商品id : 1   买2个    实际库存3
             * 库存数量 >= 递减数量 李四  商品id : 1   买2个    实际库存3
             * 这种方式 ，会产生超卖现象
             */
...
//            }
             ...
             *
             * 数据库中每条记录都有行级锁，此时只能允许一个事务修改该记录，只有等该事务结束后，
             * 其余事务才能继续修改该记录.
             */
